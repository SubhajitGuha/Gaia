...
ivec3 base_grid_coord(in DdgiParameters ddgi, vec3 X) 
{
    return clamp(ivec3((X - ddgi.gridStartPos) / ddgi.gridStep), ivec3(0, 0, 0), ivec3(ddgi.probeCounts) - ivec3(1, 1, 1));
}

// ------------------------------------------------------------------------

vec3 grid_coord_to_position(in DdgiParameters ddgi, ivec3 c)
{
    return ddgi.gridStep * vec3(c) + ddgi.gridStartPos;
}

// ------------------------------------------------------------------------

int grid_coord_to_probe_index(in DdgiParameters ddgi, in ivec3 probe_coords) 
{
    return int(probe_coords.x + probe_coords.y * ddgi.probeCounts.x + probe_coords.z * ddgi.probeCounts.x * ddgi.probeCounts.y);
}

// ------------------------------------------------------------------------

ivec3 probe_index_to_grid_coord(in DdgiParameters ddgi, int index)
{
    ivec3 i_pos;

    // Slow, but works for any # of probes
    i_pos.x = index % ddgi.probeCounts.x;
    i_pos.y = (index % (ddgi.probeCounts.x * ddgi.probeCounts.y)) / ddgi.probeCounts.x;
    i_pos.z = index / (ddgi.probeCounts.x * ddgi.probeCounts.y);
    ...
    //    i_pos.x = index & (ddgi.probeCounts.x - 1);
    //    i_pos.y = (index & ((ddgi.probeCounts.x * ddgi.probeCounts.y) - 1)) >> findMSB(ddgi.probeCounts.x);
    //    i_pos.z = index >> findMSB(ddgi.probeCounts.x * ddgi.probeCounts.y);
...
vec3 probe_location(in DdgiParameters ddgi, int index)
{
...
vec3 sample_irradiance(in DdgiParameters ddgi, vec3 P, vec3 N, vec3 Wo, sampler2D irradiance_texture, sampler2D depth_texture)
{
    ...
    vec3 alpha = clamp((P - base_probe_pos) / ddgi.gridStep, vec3(0.0f), vec3(1.0f));
    ...
    for(int i=0; i<8; ++i)
    {
        ...
        ivec3  probe_grid_coord = clamp(base_grid_coord + offset, ivec3(0), ddgi.probeCounts - ivec3(1));
        ...
        vec3 probe_to_point = P - probe_pos + (N + 3.0 * Wo) * ddgi.normalBias;
        ...
        {
            vec2 tex_coord = texture_coord_from_direction(-dir, p, ddgi.depthTextureWidth, ddgi.depthTextureHeight, ddgi.depthProbeSideLength);
        ...
        vec2 tex_coord = texture_coord_from_direction(normalize(irradiance_dir), p, ddgi.irradianceTextureWidth, ddgi.irradianceTextureHeight, ddgi.irradianceProbeSideLength);
    ...
    //#endif
    net_irradiance *= ddgi.energyPreservation;
