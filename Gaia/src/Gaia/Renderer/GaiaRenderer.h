...
namespace Gaia
{
	...
	enum BufferusageBits : uint8_t
	{
		...
		BufferUsageBits_Uniform = 1 << 3,
		BufferUsageBits_Storage = 1 << 4,
		BufferUsageBits_Indirect = 1<<5,
	...
	struct VertexInput final
	{
		...
		uint32_t GetVertexSize() const
		{
			...
			{
				vertexSize += getVertxFormatSize(attributes[i].format);
			}
			return vertexSize;
		}
